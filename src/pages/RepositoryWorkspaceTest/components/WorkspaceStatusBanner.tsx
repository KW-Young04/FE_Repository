    ...
    <>
      ...
      {truncatedCount > 0 && (
        <section className="mb-4 border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700">
          파일 수가 많아 처음 {MAX_INITIAL_FILES}개만 로드했습니다. 필요한 파일은 클릭 시 추가
          로드됩니다. (생략: {truncatedCount}개)
        </section>
      )}
      ...
      <section className="mb-4 border border-slate-200 bg-white px-4 py-3 text-xs text-slate-600">
        ...
        <p className="mt-1">
          트리: {formatDuration(diagnostics.treeMs)} / 핵심 파일:{" "}
          {formatDuration(diagnostics.coreMs)} / 런타임: {formatDuration(diagnostics.runtimeMs)} /
          백그라운드: {formatDuration(diagnostics.backgroundMs)}
        </p>
        <p>
        ...
        {diagnostics.lastError && (
          <p className="text-rose-600">마지막 오류: {diagnostics.lastError}</p>
        )}
      </section>
    </>
  );
